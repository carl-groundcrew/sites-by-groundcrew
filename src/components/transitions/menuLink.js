...
class pageLink extends React.Component {
  exit(exit, node) {
    ...
    node.querySelector('.secondary-menu').classList.remove('active');
    node.querySelector('.page-link[data-target="'+this.props.target+'"]').classList.add('active');

    const tl = gsap.timeline({paused: true});
    tl.staggerTo(document.querySelectorAll('.overlay-menu .main-navigation .page-link'), .5, {y:'10px', opacity:0, ease: 'power1.out',}, 0.1);
  ...
  render() {
    ...
    return (
      <TransitionLink className='page-link' data-target={this.props.target} exit={{ target:this.props.target, length:2.5, trigger: ({ exit, node }) => this.exit(exit, node)}} entry={{ delay:1.5, length:2.5, trigger: ({ entry, node }) => this.enter(entry, node)}} to={this.props.to}>
        {this.props.children}
        {arrow()}
      </TransitionLink>
    );
